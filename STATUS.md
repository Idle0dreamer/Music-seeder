...
prototypes. They are retained only for diagnosis and are not evidence of a
completed maqam generator or hammered dulcimer.

Current phase: **non-ML operator-algebra kernel implementation**.

The legacy generator and synthesizer remain frozen. Implementation is active
only under `include/mq/kernel`, `src/kernel`, `apps/kernel`, and
`tests/kernel`.
