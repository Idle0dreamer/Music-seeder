...
void test::feasibility() {
    ...
    chain.bound({{{y, Rational(1)}}, {}, "y at or below zero"});
    chain.bound({
        {{x, Rational(-1)}},
    ...
    symbolic.declare(x);
    symbolic.equate({{{x, Rational(1)}}, fourth, "exact fourth"});
    symbolic.bound({
        {{x, Rational(1)}},
    ...
    boundary.declare(x);
    boundary.equate({{{x, Rational(1)}}, fourth, "exact fourth"});
    ...
    pitch::System unsafe;
    unsafe.declare(x);
    unsafe.equate({{{x, Rational(1)}}, {}, "zero anchor"});
    unsafe.bound({
        {{x, Rational(-1)}},
        pitch::Expression::cents(Rational(-1)),
        "one cent floor",
    ...
    pitch::System absent;
    absent.declare(x);
    absent.bound({{{unknown, Rational(1)}}, {}, "unknown role"});
